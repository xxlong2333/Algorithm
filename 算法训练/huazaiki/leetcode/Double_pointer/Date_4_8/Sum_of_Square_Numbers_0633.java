...
/*
    ...
    Sum of Square Numbers -- 633. Sum of Square Numbers (Easy)
...
* */
public class Sum_of_Square_Numbers_0633 {
    public static void main(String[] args) {
        int target = 2147483600;
        Solution1 s1 = new Solution1();
        System.out.println(s1.judgeSquareSum(target));
    }
}
...
class Solution1 {
    public boolean judgeSquareSum(int target) {
        ...
        long i = 0, j = (long) Math.sqrt(target);   //注意类型需要用long
        while (i <= j) {
            long powSum = i * i + j * j;
            if (powSum == target) {
                return true;
